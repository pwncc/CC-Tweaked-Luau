...
{
    // CC-PATCH: as in Lua 5.3+, %q accepts nil, booleans and numbers, which
    // are emitted as literals (readable back by Lua) without quoting.
    if (arg <= lua_gettop(L))
    {
        int t = lua_type(L, arg);
        if (t == LUA_TNIL || t == LUA_TBOOLEAN || t == LUA_TNUMBER || t == LUA_TINTEGER)
        {
            size_t ll;
            const char* ls = luaL_tolstring(L, arg, &ll);
            luaL_addlstring(b, ls, ll);
            lua_pop(L, 1);
            return;
        }
    }
...
static int str_format(lua_State* L)
{
    ...
    while (strfrmt < strfrmt_end)
    {
        ...
        {                          // format item
            ...
            case 's':
            {
                // CC-PATCH: coerce non-string values (honouring __tostring),
                // as Lua 5.2+ does. Replace the argument in-place so the
                // string stays anchored while the buffer is built.
                if (arg <= lua_gettop(L) && !lua_isstring(L, arg))
                {
